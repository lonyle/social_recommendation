...
EPSILON_SCALAR = 10 # how many n is considered a step in the epsilon greedy
...
def state_param_to_key(state_param):
	...
	return str(big_informed_index) + ',' + str(big_waiting_informed_index) 
...
class State():
	...
	def best_action(self, epsilon): # UCB-style or epsilon-greedy style
		if len(self.unexplored_actions) > 0:
			if len(self.unexplored_actions) == len(possible_actions):
...
def decision_func(state_param):
	...
	if key in state_dict:		
		...
		print (state_param)
		return np.random.choice(possible_actions, p=default_weight_vec)
