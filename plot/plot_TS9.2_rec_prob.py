...
result = json.load(open('data/result_exp9.2.json'))
result_TS = json.load(open('data/result_exp9.2_TS.json'))

for key in result:
	result[key] = result[key][:3]

# for key in result_TS:
# 	result_TS[key] = result_TS[key][:3]

delta_vec = [0.04, 0.08, 0.2]#result['param_vec']
...
rects3 = ax.bar(ind+1*width, result['opt_mixed_vec'], width, label='opt. mixed', \
	fill=False)

rects2 = ax.bar(ind+2*width, result_TS['TS_mixed_vec'], width, label='mixed(TS)', color='black',\
	fill=False, hatch='/')

rects3 = ax.bar(ind+3*width, result_TS['TS_q_learning_vec'], width, label='q-learning(TS)',\
...
plt.xlabel('rec. prob. $q$', weight='bold')
...
plt.savefig('images/plot_TS_9.2_rec_prob.eps', dpi=1200)
plt.show()